...
class SheetOnion(tk.Tk):
    ...
    def _auto_magic_opcode_aob(self, target_size):
        """Kullanıcının seçtiği boyuta göre (8 veya 32) otomatik wildcard'lı AOB üretir."""
        ...
        # Seçilen boyuta göre hafızadan baytları çekiyoruz
        code_bytes = winmem.read_bytes(self.scanner.handle, addr, target_size)
        if not code_bytes:
            messagebox.showerror("AOB Hatası", f"{target_size}-Byte işlem kodları RAM'den otomatik okunamadı.")
            return
        ...
        if Cs is not None:
            ...
            # Akıllı Otomatik Maskeleme
            # İlk 4 byte'ı sabit tutup, kalan kısımları dinamik wildcard (??) haline getirme kuralı
        ...
        # Eğer capstone tam maskeleme yapamadıysa veya liste boyutu eksik kaldıysa 
        # kalan boşlukları otomatik olarak ?? ile dolduruyoruz
        while len(aob_parts) < target_size:
            aob_parts.append("??")
        ...
        self._set_status(f"⚡ Otomatik {target_size}-Byte AOB Kopyalandı!")
        msg = f"Başarılı! {target_size}-Byte maskelenmiş imza otomatik hazırlandı ve panoya kopyalandı:\n\n{magic_aob}"
        messagebox.showinfo(f"Sheet Onion - {target_size}-Byte Engine", msg)
