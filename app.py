...
class OffsetViewerDialog(tk.Toplevel):
    def __init__(self, master, process_handle, pid, addr, type_name):
        ...
        ttk.Label(
            ...
            text="'Yakalamayı Başlat' butonuna basın, sonra HEMEN oyuna geçip bu değeri değiştirecek "
                 ...
                 "yakalayıp, kalıcı kod-AOB imzaları üretecek."
        ).pack(fill="x")
    ...
    def _resolve_and_show(self, hit_addr, is64):
        ...
        # 1. KISA AOB ÜRET (Yaklaşık 16 Byte)
        short_aob, disasm_lines, err = build_code_aob(self.process_handle, hit_addr, is64, total_size=16)
        
        # Disassembly logunu ekrana yazdır
        for line in disasm_lines:
            self._log(line)

        # 2. UZUN AOB ÜRET (Yaklaşık 32 Byte)
        long_aob, _, _ = build_code_aob(self.process_handle, hit_addr, is64, total_size=32)

        if err:
            self._log(f"[!] {err}")
            self.status_lbl.config(text=err, foreground="#d0686b")
            
        if short_aob and long_aob:
            self.short_aob = short_aob
            self.long_aob = long_aob
            
            self._log("\n--- ÜRETİLEN İMZALAR ---")
            self._log(f"[KISA AOB]:\n{short_aob}\n")
            self._log(f"[UZUN AOB]:\n{long_aob}\n")
            
            self.status_lbl.config(text="Tamamlandı! İstediğiniz AOB'yi kopyalayabilirsiniz.", foreground=OK)
            self.copy_short_btn.config(state="normal")
            self.copy_long_btn.config(state="normal")

            # Varsayılan olarak uzun olanı panoya alalım
            self.clipboard_clear()
