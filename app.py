...
class SheetOnion(tk.Tk):
    ...
    def _build_table(self):
        ...
        ttk.Button(btns, text="💾 Save Table", command=self._save_table).pack(side="right", padx=(4, 0))
        ttk.Button(btns, text="📂 Load Table", command=self._load_table).pack(side="right")
    ...
    def _save_table(self):
        ...
        try:
            with open(file_path, "w", encoding="utf-8") as f:
                json.dump({"version": "1.0", "entries": data}, f, indent=4, ensure_ascii=False)
...
class PointerScannerDialog(tk.Toplevel):
    def __init__(self, master, process_handle, pid, target_addr, type_name):
        super().__init__(master)
        self.title("✦ Sheet Onion - Tam Doğru (Multi-Level) Pointer Engine")
        ...
        lbl = ttk.Label(self, text=f"Multi-Level Pointer Discovery -> Hedef Adres: 0x{target_addr:X}", font=HEADFONT, padding=10)
        lbl.pack(anchor="w")
        ...
        for col, txt, w, anchor in (("base", "Statik Base Pointer Adresi", 320, "w"), ("offsets", "Offsets", 160, "center"), ("live_val", "Live Value", 160, "e")):
        ...
        self.status_lbl = ttk.Label(b_panel, text="Hazır. 'Scan Pointer Map' butonuna basarak Level-1 ve Level-2 analizi başlatın.", foreground=MUTED)
    ...
    def _resolve_pointer_real(self):
        ...
        # 1. HATA DÜZELTME: Python'un değil, HEDEF OYUNUN (Target) mimarisini kontrol etmeliyiz.
        ...
        # 2. HATA DÜZELTME: Offset aralığını genişlettik (Özellikle Unreal Engine / Unity oyunları için)
        max_offset = 0x8000
        ...
        for cand in l1_candidates:
            ...
            if hit:
                # Level 1'de direkt olarak Statik bir adres bulundu! (Örn: game.exe+0xABC)
                ...
                # Bulunan adres yine dinamik yığında (Heap), Seviye 2 için ayır.
                dynamic_l1.append(cand)
                
        # --- SEVİYE 2 (LEVEL 2) TARAMA ---
        # 3. HATA DÜZELTME: L2 taraması için 500 çok düşüktü. Hedefi büyüterek statik adresi kaçırmayı engelliyoruz.
        ...
        if not self._scan_cancelled and dynamic_l1:
            ...
            for cand2 in l2_candidates:
                ...
                if hit2: # İkinci seviyede Statik modül (.exe veya .dll) bulunduysa
        ...
        self.found_paths = static_paths[:150]
    ...
    def _scan_memory_for_pointers(self, target_addrs, max_offset, ptr_size, np_dtype, array_typecode):
        ...
        # 4. HATA DÜZELTME: 20.000 kapasite yüzünden oyunun bellek sonuna / .exe kısmına erişilemiyordu! 
        # Kapasiteyi artırdık.
        MAX_CANDIDATES = 1_000_000
        
        # 5. HATA DÜZELTME: writable_only parametresini False yaptık. 
        # Çünkü oyunların statik adresleri bazen Sadece Okunabilir (.rdata) sayfalardadır!
    ...
    def _finish_pointer_scan(self):
        ...
        if self._scan_cancelled:
            ...
            self.status_lbl.config(text=f"Tarama Tamamlandı! {len(self.found_paths)} adet kesin/kalıcı (Statik) pointer zinciri bulundu.", foreground=OK)
        else:
    ...
    def _do_rescan(self):
        ...
        if not target_val:
            messagebox.showwarning("Sheet Onion", "Filtreleme için oyundaki yeni güncel değeri yazın.")
            return

        valid_paths = []
        
        # 6. HATA DÜZELTME: Rescan yapılırken yine hedefin mimarisini doğru almalıyız.
        is_64 = winmem.target_is_64bit(self.process_handle)
        ...
        self.status_lbl.config(text=f"Rescan (Yeniden Tarama) bitti! Geriye {len(valid_paths)} sağlam adres kaldı.", foreground=OK)
    ...
    def _save_pointer(self, event):
        ...
        messagebox.showinfo("Sheet Onion", "Pointer adresi başarıyla kaydedildi!")
    ...
    def __init__(self, master, process_handle, pid, addr, type_name):
        ...
        ttk.Label(
            ...
            text="'Yakalamayı Başlat' butonuna basın, ardından oyuna geçip değeri değiştirin/erişin. "
                 "Hata ayıklayıcı gerçek zamanlı olarak her erişen komutu sayaçla (Count) ve Offset'iyle listeleyecektir."
        ).pack(fill="x")
    ...
    def _handle_hit(self, hit_addr):
        ...
        if hit_addr in self.hit_map:
            ...
            if raw and Cs is not None:
                md = Cs(CS_ARCH_X86, CS_MODE_64 if is64 else CS_MODE_32)
                md.detail = True # Otomatik Offset bulmak için detay modunu açıyoruz
                try:
                    for insn in md.disasm(raw, hit_addr):
                        ...
                        # --- OTOMATİK OFFSET BULMA (CAPSTONE İLE) ---
                        extracted_offset = None
                        base_register = None
                        
                        for op in insn.operands:
                            if op.type == X86_OP_MEM: # Bellek operasyonu mu?
                                if op.mem.base != 0:
