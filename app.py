...
class SheetOnion(tk.Tk):
    ...
    def _generate_aob_by_size(self, addr, size):
        """Belirtilen uzunlukta (32 veya 64) maskelenmiş gerçek AOB imzası üretir."""
        ...
        aob_parts = []
        # Dinamik maskeleme eşiği: Toplam uzunluğun son çeyreğini hizalama alanı say
        mask_threshold = int(size * 0.75)
        
        for idx, b in enumerate(buf):
            if (idx >= mask_threshold and b == 0x00) or (b == 0xCC): 
                aob_parts.append("??")
            else:
        ...
        self._set_status(f"AOB ({size}-Byte) Copied!")
        messagebox.showinfo("Sheet Onion - AOB Engine", f"{size}-Byte Benzersiz AOB imzası üretildi ve panoya kopyalandı:\n\n{real_aob}")

    def _show_results_context_menu(self, event):
        """Sonuç listesinde sağ tıklandığında açılan gelişmiş 32/64-Byte AOB seçim menüsü"""
        ...
        res_menu.add_command(label="🎯 Otomatik Gerçek AOB Bul (32-Byte Standart)", command=lambda: self._generate_aob_by_size(addr, 32))
        res_menu.add_command(label="🚀 Otomatik Gerçek AOB Bul (64-Byte Ultra Benzersiz)", command=lambda: self._generate_aob_by_size(addr, 64))
    ...
    def _toggle_freeze_space(self, event):
        # Düzeltilen sözdizimi (SyntaxError çözümü)
        sel = self.table.selection()
