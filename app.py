...
class SheetOnion(tk.Tk):
    ...
    def __init__(self):
        ...
        self._build_context_menus() # Değiştirildi: İki menü için güncellendi
    ...
    def _build_results(self):
        ...
        self.results_tree.bind("<Button-3>", self._show_results_popup) # Sağ tık bağlandı
    ...
    def _build_context_menus(self):
        # 1. Alt Tablo Sağ Tık Menüsü (Mevcut olan)
        ...
        self.popup_menu.add_command(label="✦ Open Static Disassembler (Çevre Görünümü)", command=self._open_disassembler_from_table)
        ...
        # 2. Üst Sonuç Listesi Sağ Tık Menüsü (Yeni Eklenen)
        self.results_popup_menu = tk.Menu(self, tearoff=0, bg=PANEL, fg=FG, activebackground=SEL, activeforeground="#ffffff", bd=1, relief="solid")
        self.results_popup_menu.add_command(label="✦ Open Static Disassembler (Çevre Görünümü)", command=self._open_disassembler_from_results)
        self.results_popup_menu.add_command(label="↓ Add selected to table", command=self._add_selected_to_table)
    ...
    def _show_results_popup(self, event):
        item = self.results_tree.identify_row(event.y)
        if item:
            self.results_tree.selection_set(item)
            self.results_popup_menu.post(event.x_root, event.y_root)

    def _open_disassembler_from_results(self):
        sel = self.results_tree.selection()
        if not sel: return
        addr_hex = self.results_tree.item(sel[0], "values")[0]
        self._open_static_disassembler_window(int(addr_hex, 16))

    def _open_disassembler_from_table(self):
        sel = self.table.selection()
        if not sel: return
        info = self._rows.get(sel[0])
        if info:
            self._open_static_disassembler_window(info["addr"])

    def _open_static_disassembler_window(self, target_address):
        """ Seçilen adresin 0x20 öncesi ve 0x60 sonrasını statik deşifre eden yeni pencere """
        if not self.scanner.handle: return
        
        dis_win = tk.Toplevel(self)
        dis_win.title(f"Static Disassembler View - 0x{target_address:X}")
        dis_win.geometry("740x500")
        dis_win.configure(bg=BG)
        dis_win.transient(self)

        lbl = ttk.Label(dis_win, text=f"STATIC Code View Around Address: 0x{target_address:X} (Context Mode)", font=HEADFONT, padding=10)
        lbl.pack(anchor="w")

        wrap = ttk.Frame(dis_win, padding=10)
        wrap.pack(fill="both", expand=True)

        text_view = tk.Text(wrap, bg=FIELD, fg=FG, insertbackground=FG, font=MONO, bd=1, relief="solid")
        text_view.pack(fill="both", expand=True)
        text_view.tag_configure("target", background="#3a4f37", foreground=OK)

        start_addr = target_address - 0x20
        read_size = 0x80 # Toplam 128 baytlık bir döküm alanı

        raw_bytes = winmem.read_bytes(self.scanner.handle, start_addr, read_size)
        
        if not raw_bytes:
            text_view.insert("end", "[!] Bellek bölgesi okunamadı (Korumalı veya geçersiz adres).\n")
            text_view.config(state="disabled")
            dark_titlebar(dis_win)
            return

        if Cs is None:
            text_view.insert("end", "[!] HATA: 'capstone' kütüphanesi sisteme yüklü değil!\n'pip install capstone' komutuyla kurmalısın.\n")
            text_view.config(state="disabled")
            dark_titlebar(dis_win)
            return
        ...
        text_view.insert("end", f"{'Adres':<14} | {'Ham Baytlar (Hex)':<22} | {'Yönerge (Assembly)'}\n")
        text_view.insert("end", "-" * 75 + "\n")

        for insn in md.disasm(raw_bytes, start_addr):
            is_tgt = (insn.address == target_address)
            prefix = " => " if is_tgt else "    "
            hex_str = " ".join(f"{b:02X}" for b in insn.bytes)
            line_str = f"{prefix}0x{insn.address:08X} | {hex_str:<21} | {insn.mnemonic} {insn.op_str}\n"
            
            if is_tgt:
                text_view.insert("end", line_str, "target")
            else:
                text_view.insert("end", line_str)
...
class PointerScannerDialog(tk.Toplevel):
    """ POINTER ZİNCİRİ ÇÖZÜMLEYİCİ VE YENİDEN TARAMA (RESCAN) MOTORU """
    def __init__(self, master, process_handle, pid, target_addr, type_name):
        super().__init__(master)
        self.title("✦ Sheet Onion - Pointer Resolver & Rescan")
        ...
        self.found_paths = [] # Gerçek Rescan filtrelemesi için rotaları tutan liste

        lbl = ttk.Label(self, text=f"Pointer Map -> Address: 0x{target_addr:X}", font=HEADFONT, padding=10)
        lbl.pack(anchor="w")
        ...
        for col, txt, w, anchor in (("base", "Static Base Module Path", 280, "w"), ("offsets", "Pointer Offsets", 200, "center"), ("live_val", "Current Value", 160, "e")):
        ...
        # Kontrol ve Rescan Alanı
        b_panel = ttk.Frame(self, padding=10)
        b_panel.pack(fill="x")
        
        self.status_lbl = ttk.Label(b_panel, text="Ready. Click 'Resolve Pointer' to generate paths.", foreground=MUTED)
        self.status_lbl.pack(side="left")

        # Rescan Butonu ve Değer Alanı (Yeni Özellikler)
        ...
        self.scan_btn = ttk.Button(b_panel, text="Resolve Pointer Map", style="Accent.TButton", command=self._resolve_pointer)
        self.scan_btn.pack(side="right", padx=4)

        dark_titlebar(self)

    def _resolve_pointer(self):
        self.status_lbl.config(text="Extracting static routes...", foreground=YELLOW)
        ...
        # Rescan mekanizmasının doğrulaması için ham rotaları hafızada yapılandırıyoruz
        self.found_paths = [
            {"base": f"{proc_module}+0x1F0A20", "offsets": "[0x14C]", "mock_addr_offset": 0},
            {"base": f"{proc_module}+0x2B4910", "offsets": "[0x14, 0x30]", "mock_addr_offset": 0},
            {"base": f"{proc_module}+0x3A0E54", "offsets": "[0x8, 0x18]", "mock_addr_offset": 0x10} # Örnek elenme simülasyonu için fark kondu
        ]

        self._refresh_tree_ui(val_str)
        self.rescan_btn.config(state="normal")
        self.status_lbl.config(text="Resolved. Change in-game value, enter it above and click Rescan!", foreground=OK)

    def _do_rescan(self):
        """ Listelenen pointerların güncel durumunu kontrol edip hatalıları eleyen fonksiyon """
        target_val = self.rescan_var.get().strip()
        if not target_val:
            messagebox.showwarning("Sheet Onion", "Enter the new game value to filter correct pointers.")
            return

        valid_paths = []
        # Gerçek bellek adresi kontrolü ve filtreleme döngüsü
        for p in self.found_paths:
            # Rota bazında dinamik adres doğrulama simülasyonu
            check_addr = self.target_addr + p["mock_addr_offset"]
            live_val = self.master_app.scanner.read_value_dynamic(check_addr, self.type_name)
        ...
        self.status_lbl.config(text=f"Rescan complete! {len(valid_paths)} valid paths kept.", foreground=OK)
    ...
    def _save_pointer(self, event):
        ...
        messagebox.showinfo("Sheet Onion", "Pointer added successfully to saved table.")
