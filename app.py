...
class SheetOnion(tk.Tk):
    ...
    def _build_context_menu(self):
        ...
        # Alt Tablo için Gelişmiş AOB Alt Menüsü
    ...
    def _attach_aob_commands(self, menu_obj, is_table=False):
        """Menülere 8, 10, 32 ve 64 byte'lık dinamik AOB komutlarını bağlar."""
        sizes = [8, 10, 32, 64]
        for s in sizes:
    ...
    def _trigger_aob_generation(self, size, use_wildcards, from_table=False):
        """Tıklanan listenin durumuna göre doğru adresi çeker ve AOB motorunu çalıştırır."""
        if from_table:
    ...
    def _generate_aob_by_size(self, addr, size, use_wildcards=True):
        """Belirtilen uzunlukta maskelenmiş veya ham AOB imzası üretir."""
    ...
    def _show_results_context_menu(self, event):
        """Üst sonuç listesinde sağ tıklandığında açılan gelişmiş AOB seçim menüsü"""
