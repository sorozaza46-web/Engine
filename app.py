    ...
    try:
        ...
        for attr in (20, 19):
            if dwm.DwmSetWindowAttribute(
                hwnd, attr, ctypes.byref(enable), ctypes.sizeof(enable)
            ) == 0:
...
def make_logo(parent, px=46, bg=BG):
    ...
    poly([(50, 24), (58, 42), (44, 60), (56, 80), (50, 94),
          (34, 86), (19, 60), (23, 39)], BLUE)
    poly([(50, 24), (78, 39), (82, 60), (66, 86), (50, 94),
          (56, 80), (44, 60), (58, 42)], YELLOW)
...
class ProcessDialog(tk.Toplevel):
    ...
    def __init__(self, master):
        ...
        self.tree = ttk.Treeview(
            wrap, columns=("pid", "name"), show="headings", selectmode="browse"
        )
        ...
        btns.pack(fill="x")
        ttk.Button(btns, text="Attach", style="Accent.TButton",
...
class SheetOnion(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("Sheet Onion - Temu Cheat Engine")
    ...
    def _apply_theme(self):
        ...
        style.map("TRadiobutton",
                  ...
                  indicatorcolor=[("selected", BLUE), ("!selected", FIELD)])

        style.configure("TButton", background=PANEL, foreground=FG,
                        borderwidth=0, relief="flat", padding=(11, 6), focuscolor=BG)
        style.map("TButton",
                  background=[("pressed", "#3a3e44"), ("active", "#34373c"),
                              ("disabled", "#222427")],
                  foreground=[("disabled", "#5c5f63")])
        style.configure("Accent.TButton", background=BLUE, foreground="#0c1116",
                        font=HEADFONT, padding=(13, 6))
        style.map("Accent.TButton",
                  background=[("pressed", "#3d76a3"), ("active", "#5a9bcd"),
                              ("disabled", "#2a3d4d")],
                  foreground=[("disabled", "#7d8a93")])

        for name in ("TEntry", "TCombobox"):
            style.configure(name, fieldbackground=FIELD, foreground=FG,
                            bordercolor=BORDER, borderwidth=1, relief="flat",
                            insertcolor=FG, arrowcolor=MUTED, padding=4)
        ...
        style.configure("Treeview", background=PANEL, fieldbackground=PANEL,
                        ...
                        rowheight=25, font=MONO)
        style.map("Treeview",
                  background=[("selected", SEL)], foreground=[("selected", "#ffffff")])
        style.layout("Treeview", [("Treeview.treearea", {"sticky": "nswe"})])
        style.configure("Treeview.Heading", background=HEAD, foreground=MUTED,
                        relief="flat", borderwidth=0, padding=(8, 7), font=HEADFONT)
        style.map("Treeview.Heading",
                  background=[("active", "#34373c")], foreground=[("active", FG)])
        ...
        style.configure("Vertical.TScrollbar", background=THUMB, troughcolor=BG,
                        bordercolor=BG, borderwidth=0, relief="flat", width=12)
        style.map("Vertical.TScrollbar", background=[("active", "#4c515a")])

        style.configure("Status.TLabel", background="#141517", foreground=MUTED,
                        font=UIFONT)
    ...
    def _build_toolbar(self):
        ...
        ttk.Label(bar, text="Value:").grid(row=1, column=0, sticky="w", pady=(10, 0))
        ...
        tframe = ttk.Frame(bar)
        tframe.grid(row=1, column=2, sticky="w", padx=8, pady=(10, 0))
        ttk.Radiobutton(tframe, text="4 Bytes", value="4 Bytes",
                        variable=self.type_var).pack(side="left")
        ttk.Radiobutton(tframe, text="Float", value="Float",
                        variable=self.type_var).pack(side="left", padx=6)

        ttk.Label(bar, text="Scan:").grid(row=1, column=3, sticky="e", pady=(10, 0))
        self.mode_var = tk.StringVar(value=scanner.EXACT)
        self.mode_combo = ttk.Combobox(
            bar, textvariable=self.mode_var, values=scanner.SCAN_MODES,
            state="readonly", width=14,
        )
        self.mode_combo.grid(row=1, column=4, sticky="w", padx=4, pady=(10, 0))

        btns = ttk.Frame(bar)
        btns.grid(row=2, column=0, columnspan=5, sticky="w", pady=(12, 0))
        self.first_btn = ttk.Button(btns, text="First Scan", style="Accent.TButton",
                                    command=self._scan_clicked)
        self.first_btn.pack(side="left")
        self.next_btn = ttk.Button(btns, text="Next Scan", style="Accent.TButton",
                                   command=self._next_clicked, state="disabled")
        self.next_btn.pack(side="left", padx=6)
        self.new_btn = ttk.Button(btns, text="New Scan", command=self._new_scan,
                                  state="disabled")
    ...
    def _build_results(self):
        ...
        self.results_tree = ttk.Treeview(
            wrap, columns=("addr", "value", "prev"), show="headings"
        )
        for col, txt, w, anchor in (("addr", "Address", 200, "w"),
                                    ("value", "Value", 110, "e"),
                                    ("prev", "Previous", 110, "e")):
        ...
        ttk.Button(frame, text="↓  Add selected to table",
                   command=self._add_selected_to_table).pack(side="bottom", anchor="w",
                                                              pady=(6, 0))
    ...
    def _build_table(self):
        ...
        ttk.Button(btns, text="Add address manually",
                   command=self._add_manual).pack(side="left")
        ttk.Button(btns, text="Edit value",
                   command=self._do_edit_value).pack(side="left", padx=6)
    ...
    def attach(self):
        ...
        if tgt64 and not winmem.HOST_IS_64BIT:
            ...
            messagebox.showerror(
                "Sheet Onion",
                f"{name} is a 64-bit process but you're running 32-bit Python.\n"
                "Run Sheet Onion with 64-bit Python to scan 64-bit targets.",
            )
            return

        if self.scanner.handle:
            winmem.close_process(self.scanner.handle)
    ...
    def _scan_clicked(self):
        if not self._require_attached() or self.scanning:
            return
        try:
            value = scanner.parse_value(self.value_var.get(), self.type_var.get())
        except ValueError:
            messagebox.showerror("Sheet Onion", "Enter a valid number to scan for.")
            return
        self.scanner.type_name = self.type_var.get()
        self._begin_scan("Scanning memory...")

        def work():
            n = self.scanner.first_scan(value)
    ...
    def _next_clicked(self):
        ...
        value = None
        if mode == scanner.EXACT:
            try:
                value = scanner.parse_value(self.value_var.get(), self.scanner.type_name)
            except ValueError:
                messagebox.showerror("Sheet Onion", "Enter a valid number to scan for.")
                return
        ...
        def work():
            n = self.scanner.next_scan(mode, value)
            self.after(0, lambda: self._end_scan(n))
    ...
    def _begin_scan(self, msg):
        self.scanning = True
        for b in (self.first_btn, self.next_btn, self.new_btn, self.attach_btn):
    ...
    def _end_scan(self, count):
        ...
        extra = "  (truncated - refine your search)" if self.scanner.truncated else ""
    ...
    def _populate_results(self):
        ...
        if needle:
            rows = []
            for addr, val in self.scanner.iter_all():
                if needle in f"{addr:x}":
                    rows.append((addr, val))
                    if len(rows) >= self.DISPLAY_LIMIT:
                        break
            note = f" (matched {len(rows)} of {total} by filter)"
        else:
            rows = self.scanner.head(self.DISPLAY_LIMIT)
            note = "" if total <= self.DISPLAY_LIMIT else f" (showing first {self.DISPLAY_LIMIT})"

        for i, (addr, val) in enumerate(rows):
            self.results_tree.insert(
                "", "end", tags=(row_tag(i),),
                values=(f"{addr:X}", self._fmt_value(val), self._fmt_value(val)),
            )
        self.count_label.config(text=f"Found: {total}{note}")
    ...
    def _add_selected_to_table(self):
        ...
        for item in self.results_tree.selection():
            addr_hex, _value, _prev = self.results_tree.item(item, "values")
            added.append(self._add_table_row(int(addr_hex, 16),
                                             self.scanner.type_name, ""))
        if added:
            self.table.selection_set(added)
            self.table.focus(added[-1])
            self.table.see(added[-1])

    def _add_manual(self):
        if not self._require_attached():
            return
        text = ask_string(self, "Add address", "Address (hex, e.g. 1A2B3C):")
        if not text:
            return
        try:
            addr = int(text.strip(), 16)
        except ValueError:
            messagebox.showerror("Sheet Onion", "Invalid hex address.")
            return
    ...
    def _add_table_row(self, addr, type_name, desc):
        ...
        item = self.table.insert("", "end", tags=(tag,),
                                 values=(desc, f"{addr:X}", type_name, "?"))
    ...
    def _do_edit_value(self):
        ...
        if not info or not self.scanner.handle:
            return
        cur = self.table.item(item, "values")[3]
        new = ask_string(self, "Set value", f"New value for {info['addr']:X}:",
                         initial="" if cur in ("?", "??") else cur)
        if new is None:
            return
        try:
            value = scanner.parse_value(new, info["type"])
        except ValueError:
            messagebox.showerror("Sheet Onion", "Invalid value.")
            return
        saved = self.scanner.type_name
        self.scanner.type_name = info["type"]
        ok = self.scanner.write_value(info["addr"], value)
        self.scanner.type_name = saved
        if not ok:
            messagebox.showerror("Sheet Onion", "Write failed (need admin / bad address?).")
    ...
    def _refresh_table_values(self):
        ...
        for item, info in self._rows.items():
            self.scanner.type_name = info["type"]
            val = self.scanner.read_value(info["addr"])
            self.scanner.type_name = saved
