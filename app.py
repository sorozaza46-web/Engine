...
class PointerScannerDialog(tk.Toplevel):
    """ TAMAMEN ÇALIŞAN GERÇEK POINTER SCANNER VE MULTI-LEVEL RESCAN MOTORU """
    ...
    def _resolve_pointer_real(self):
        """ Cheat Engine Altyapısı: Statik modül alanlarında adres işaretçilerini arar """
        ...
        # Adres aralığı tarama limitleri (Sık kullanılan ofset derinliği: 0x0 - 0x2000)
        max_offset = 0x2000 
        discovered_ptrs = []

        # Bellekteki TÜM bölgeleri değil, sadece uygulamanın ana modül alanlarını ve okunabilir sayfaları tara
        for base, size, _, _ in winmem.iter_regions(self.process_handle, winmem.ALL_KINDS, True):
            ...
            while pos < end:
                ...
                # Her hizalanmış adreste bir pointer adresi olup olmadığını denetle
                for i in range(0, len(data) - ptr_size, ptr_size):
                    val = struct.unpack(unpack_fmt, data[i:i+ptr_size])[0]
                    
                    # Eğer bu adresten okunan değer bizim can adresimizin yakınındaysa (ofset mesafesi) yakala!
        ...
        # Arayüzü şişirmemek adına en kararlı ilk 150 rotayı listeye ekle
    ...
    def _do_rescan(self):
        """ Ayıklama Motoru: Girdiğin yeni değere uymayan sahte pointer yollarını anında siler """
