...
class PointerScannerDialog(tk.Toplevel):
    ...
    def _resolve_pointer_real(self):
        ...
        for base, size, _, _ in winmem.iter_regions(self.process_handle, winmem.ALL_KINES, True):
            pos = base
            end = base + size
            while pos < end:
...
class MemoryBrowserDialog(tk.Toplevel):
    def __init__(self, master, scanner_obj, base_addr):
        ...
        # İşlem kodları (Opcode) üzerinden AOB bulmak için yeni sağ tık menüsü
        self.hex_context_menu = tk.Menu(self.view, tearoff=0, bg=PANEL, fg=FG, activebackground=SEL, activeforeground="#ffffff")
        self.hex_context_menu.add_command(label="🎯 Bu Adresteki İşlem Kodundan (Opcode) 8-Byte AOB Üret", command=self._generate_opcode_aob_8)
        self.hex_context_menu.add_command(label="🎯 Bu Adresteki İşlem Kodundan (Opcode) 16-Byte AOB Üret", command=self._generate_opcode_aob_16)
        
        self.view.bind("<Button-3>", self._show_hex_menu)
        
        self._refresh_view()
        dark_titlebar(self)

    def _show_hex_menu(self, event):
        self.hex_context_menu.post(event.x_root, event.y_root)

    def _generate_opcode_aob_8(self):
        self._create_opcode_signature(8)

    def _generate_opcode_aob_16(self):
        self._create_opcode_signature(16)

    def _create_opcode_signature(self, size):
        if not self.scanner.handle: return
        try:
        ...
        # Kod bloğunu RAM'den çekiyoruz
        code_bytes = winmem.read_bytes(self.scanner.handle, addr, size)
        if not code_bytes:
            messagebox.showerror("AOB Error", "İşlem kodları RAM'den okunamadı.")
            return

        aob_parts = []
        # Eğer gerçek bir disassembler yüklüyse dinamik olarak maskeliyoruz, yoksa statik güvenli maske
        if Cs is not None:
            md = Cs(CS_ARCH_X86, CS_MODE_64 if winmem.HOST_IS_64BIT else CS_MODE_32)
            parsed = list(md.disasm(code_bytes, addr))
            if parsed:
                for insn in parsed:
                    # Komutun ham byte'larını ekle
                    for b in insn.bytes:
                        aob_parts.append(f"{b:02X}")
            else:
                aob_parts = [f"{b:02X}" for b in code_bytes]
        else:
            # Yedek plan: Opcode'un kendisini al, değişebilecek adres/ofset byte'larını (son 3 byte gibi) ?? yap
            for idx, b in enumerate(code_bytes):
                if idx >= int(size * 0.6): # Kodun değişken/ofset kısımlarını maskele
                    aob_parts.append("??")
                else:
                    aob_parts.append(f"{b:02X}")

        opcode_aob = " ".join(aob_parts)
        self.clipboard_clear()
        self.clipboard_append(opcode_aob)
        
        msg = f"Tebrikler! İşlem koduna (Opcode) ait {size}-Byte değişmez AOB imzası üretildi ve panoya kopyalandı:\n\n{opcode_aob}\n\nBu imzayı kullanarak dünya değişse bile ana ekrandan 'Hex / AOB' modunda arama yapabilirsin."
        messagebox.showinfo("Sheet Onion - Opcode AOB Engine", msg)
