...
def _enable_dpi_awareness():
    """Without this, on any Windows display scaled above 100% (125%/150%/
    200% - the default on most modern laptops/4K monitors), Windows scales
    the whole window as a bitmap instead of letting Tk render at native
    resolution. The visible symptom is exactly what gets reported as
    'wrong/blurry fonts': Segoe UI looks fuzzy and slightly the wrong size.
    Must be called before any Tk root window is created.
    """
...
class PointerScannerDialog(tk.Toplevel):
    ...
    def _resolve_pointer_real(self):
        ...
        # Real module base addresses - needed so the reported "base+offset"
        # is actually stable across process restarts (ASLR-safe), instead
        # of just truncating the raw runtime address.
        modules = proclist.list_modules(self.pid)

        max_offset = 0x2000
        lo = self.target_addr - max_offset
        hi = self.target_addr
        MAX_CANDIDATES = 4000  # plenty of headroom before the top-150 trim below;
                               # stops a pathological target value (e.g. near 0)
                               # from turning into an unbounded full-process scan
        discovered_ptrs = []

        regions = list(winmem.iter_regions(self.process_handle, winmem.ALL_KINDS, True))
        total_regions = len(regions)
        CHUNK = 16 * 1024 * 1024  # bigger reads = fewer ReadProcessMemory syscalls
        scanned_mb = 0.0
        last_ui_update = 0.0
            ...
            while pos < end:
                ...
                if usable_len > 0:
                    if np is not None:
                        ...
                        # Bulk byte->int conversion is still far faster than
                        # struct.unpack()-per-slot, even without numpy.
        ...
        for item in discovered_ptrs[:150]:
            ...
            if hit:
                ...
                # Pointer lives outside any loaded module (heap/stack/etc).
                # It will NOT be stable across restarts - flagged as such
                # instead of pretending it's a static module offset.
...
def _capstone_disp_info(insn):
    """Return (disp_offset, disp_size) for this instruction if Capstone's
    encoding info is available. Different Capstone binding versions expose
    it slightly differently, so both known shapes are tried; None if
    neither is present (older Capstone build without encoding detail)."""
...
def _mem_operand_is_pure_absolute(insn):
    """True only for `[[disp32]]`-style addressing with no base/index
    register at all (e.g. classic 32-bit `mov eax, [0x00458DFC]`). This is
    the one case where the address bytes are baked into the code and will
    break across ASLR/different builds. Register-relative addressing like
    `[rax+38h]` (an object-field offset) and RIP-relative addressing
    (stable within the same module) are deliberately left alone."""
    try:
...
def build_code_aob(process_handle, code_addr, is64, total_size=16):
    """Disassemble the real code at code_addr and build a masked AOB:
    opcode / ModRM / register-encoding bytes stay literal, only bytes
    that encode a pure absolute address get wildcarded (??), so the
    signature keeps matching the same instruction across restarts
    instead of matching one specific run's data value.

    Returns (aob_string_or_None, log_lines, error_or_None).
    """
    log = []
    if Cs is None:
...
class OffsetViewerDialog(tk.Toplevel):
    """'Find out what accesses this address' - sets a real hardware
    breakpoint (Dr0/Dr7) on the target address via the Windows Debug API
    and waits for a thread to actually touch it, then shows the genuine
    instruction that did and a proper masked code-AOB signature built
    from it. Change the value in-game (jump, take damage, etc.) right
    after starting the capture to trigger it.
    """
