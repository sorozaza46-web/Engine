...
def dark_titlebar(window):
    ...
    try:
        ...
        for attr in (20, 19):
            # DÜZELTME: sizeof(enable) yerine ctypes.sizeof(enable) kullanıldı.
    ...
    def _resolve_pointer_real(self):
        ...
        for cand in l1_candidates:
            ...
            if hit:
                ...
                # Bulunan adres yine dinamik yığında (Heap), Seviye 2 (Level-2) taraması için ayır.
                dynamic_l1.append(cand)
                
        # --- SEVİYE 2 (LEVEL 2) TARAMA ---
        # Performans için sadece ilk 500 dinamik hedefi geriye doğru araştırıyoruz
        dynamic_l1 = dynamic_l1[:500]
        ...
        if not self._scan_cancelled and dynamic_l1:
            ...
            for cand2 in l2_candidates:
                ...
                if hit2: # İkinci seviyede Statik modül (.exe veya .dll) bulunduysa
                    ...
                    # Bu L2 pointer'ı, hangi L1 hedefini gösteriyordu onu bulalım:
                    ...
                    if l1_info:
                        static_paths.append({
                            ...
                            "offset_val": l1_info["offset"]  # Rescan canlı testi için son offseti tutuyoruz
                        })

        # Çok fazla sonuç çıkmasını önlemek için kalıcı statik adresleri limitleyelim (Cheat Engine varsayılanı)
