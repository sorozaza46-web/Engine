...
def dark_titlebar(window):
    ...
    try:
        ...
        for attr in (20, 19):
            if dwm.DwmSetWindowAttribute(hwnd, attr, ctypes.byref(enable), sizeof(enable)) == 0:
...
class OffsetViewerDialog(tk.Toplevel):
    def __init__(self, master, process_handle, pid, addr, type_name):
        ...
        ttk.Label(
            ...
            text="'Yakalamayı Başlat' butonuna basın, ardından oyuna geçip değeri değiştirin/erişin. "
                 "Hata ayıklayıcı oyunun akışını ve arka plan iş parçacıklarını dondurmadan "
                 "gerçek zamanlı izleme yapacak ve her erişen komutu sayaçla (Count) listeleyecektir."
        ).pack(fill="x")

        self.status_lbl = ttk.Label(self, text="Hazır. İzlemeyi başlatmak için butona basın.", padding=(10, 0, 10, 6))
        self.status_lbl.pack(anchor="w")

        # Layout: Split ve Treeview
        paned = ttk.Panedwindow(self, orient="vertical")
        paned.pack(fill="both", expand=True, padx=10, pady=5)

        # Tablo Alanı (CE Tarzı Hit List)
        ...
        self.tree.column("insn", width=330, anchor="w")
        ...
        # Alt Detay Alanı (Kod analizi & AOB)
    ...
    def _handle_hit(self, hit_addr):
        ...
        # Eğer bu instruction adresi zaten listede varsa sayacı artır ve tabloyu güncelle
        if hit_addr in self.hit_map:
            ...
            # Yeni bir komut yakalandı, tek seferlik disassemble et
            is64 = winmem.target_is_64bit(self.process_handle)
            ...
            if raw and Cs is not None:
                ...
                try:
                    for insn in md.disasm(raw, hit_addr):
                        ...
                        break  # Sadece ilk komutu alıyoruz
                except Exception:
                    pass
