...
class SheetOnion(tk.Tk):
    ...
    def _build_toolbar(self):
        ...
        # Ekstra Güvenli Filtre Seçeneği (Module-Bounded Reading Entegrasyonu)
        self.mod_filter_var = tk.BooleanVar(value=False)
        self.mod_filter_chk = ttk.Checkbutton(bar, text="Sadece Ana Modülü Oku/Tara (Güvenli/Dünya Geçişi İçin)", variable=self.mod_filter_var)
        self.mod_filter_chk.grid(row=2, column=0, columnspan=3, sticky="w", pady=(6, 0))

        btns = ttk.Frame(bar)
        btns.grid(row=3, column=0, columnspan=6, sticky="w", pady=(12, 0))
        ...
        make_logo(bar, 46).grid(row=0, column=6, rowspan=4, sticky="ne", padx=(8, 0))
    ...
    def attach(self):
        ...
        # Seçime göre tarama alanlarını ayarla
        self._update_scanner_bounds(handle)
    ...
    def _update_scanner_bounds(self, handle):
        """Kullanıcının seçtiği onay kutusuna göre tarayıcı bellek sınırlarını günceller."""
        if self.mod_filter_var.get():
            # Güvenli Modül Okuma Filtresi Etkin: Sadece ana modül sınırlarında kal
            self.scanner.writable_only = False
            self.scanner.scan_kinds = ("module",) 
            self._set_status("Tarama Modu: Sadece Ana Modül Belleği Okunuyor.")
        else:
            # Önceki Klasik Mod Etkin: Tüm dinamik Private bölgeleri tarar
            self.scanner.writable_only = True
            self.scanner.scan_kinds = ("private",)
            self._set_status("Tarama Modu: Geniş Kapsamlı (Önceki Mantık) Aktif.")
    ...
    def _scan_clicked(self):
        ...
        # Tarama öncesi kullanıcının filtreyi değiştirip değiştirmediğini kontrol et
        self._update_scanner_bounds(self.scanner.handle)
