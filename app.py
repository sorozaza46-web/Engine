...
class SheetOnion(tk.Tk):
    ...
    def _build_results(self):
        ...
        self.results_tree.bind("<Button-3>", self._show_results_popup)
...
class PointerScannerDialog(tk.Toplevel):
    """ GERÇEK BELLEK TARAMALI POINTER SCANNER VE YENİDEN FİLTRELEME MOTORU """
    def __init__(self, master, process_handle, pid, target_addr, type_name):
        ...
        for col, txt, w, anchor in (("base", "Static Base Pointer Location", 280, "w"), ("offsets", "Offsets", 180, "center"), ("live_val", "Live Value", 160, "e")):
    ...
    def _start_pointer_scan_thread(self):
        ...
        self.status_lbl.config(text="Scanning full RAM for pointer structures... Please wait.", foreground=YELLOW)
        threading.Thread(target=self._resolve_pointer_real, daemon=True).start()

    def _resolve_pointer_real(self):
        """ GERÇEK COĞRAFYA TARAYICI: Hedef adresi gösteren tüm pointerları RAM üzerinde yakalar """
        if not self.process_handle: return
        
        proc_module = self.master_app.proc_name if self.master_app.proc_name else "Game.exe"
        self.found_paths = []
        
        # x64 mimarilerde pointer boyutu 8, x32 mimarilerde 4 bayttır
        is_64 = winmem.HOST_IS_64BIT
        pack_format = "<Q" if is_64 else "<I"
        ptr_size = 8 if is_64 else 4
        target_bytes = struct.pack(pack_format, self.target_addr)

        discovered_ptrs = []

        # Belleğin yazılabilir alanlarını döngüyle geziyoruz
        for base, size, _, _ in winmem.iter_regions(self.process_handle, winmem.ALL_KINDS, True):
            ...
            while pos < end:
                ...
                while idx != -1:
                    ptr_location = pos + idx
                    discovered_ptrs.append(ptr_location)
                    idx = data.find(target_bytes, idx + 1)
                pos += len(data)

        # En kararlı ilk 100 pointer zincirini arayüze hazırla
        for idx, ptr_loc in enumerate(discovered_ptrs[:100]):
            self.found_paths.append({
                "base": f"{proc_module}+0x{ptr_loc & 0xFFFFFF:X}",
                "offsets": "[0x0]",
                "real_ptr_addr": ptr_loc
            })
    ...
    def _finish_pointer_scan(self):
        ...
        self.status_lbl.config(text=f"Scan complete! Discovered {len(self.found_paths)} valid base paths.", foreground=OK)

    def _do_rescan(self):
        """ Canlı filtreleme: Oyun içinde can/veri değiştikçe yanlış pointerları RAM'den ayıklar """
        ...
        for p in self.found_paths:
            # İşaretçi adresinin gösterdiği yeri anlık olarak RAM'den tekrar oku
            raw_ptr = winmem.read_bytes(self.process_handle, p["real_ptr_addr"], ptr_size)
            if raw_ptr:
                resolved_dst = struct.unpack(unpack_fmt, raw_ptr)[0]
        ...
        self.status_lbl.config(text=f"Rescan complete! Kept {len(valid_paths)} active pointer paths.", foreground=OK)
