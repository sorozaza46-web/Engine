...
def build_code_aob(process_handle, code_addr, is64, total_size=16):
    ...
    # Uzun AOB'ler için okuma kapasitesini genişlettik (total_size + 32)
    ...
    try:
        for insn in md.disasm(raw, code_addr):
            ...
            # 1. Mutlak bellek adreslerini maskele
            ...
            # 2. CALL ve JMP komutlarının hedef adreslerini maskele
...
class OffsetViewerDialog(tk.Toplevel):
    """
    Passive Cross-Reference (X-Ref) Scanner.
    Finds instructions accessing the target address by scanning and disassembling
    the executable (.text) sections of the main process module.
    100% stable, passive, and crash-free (no debug API, no breakpoints).
    """
    def __init__(self, master, process_handle, pid, addr, type_name):
        super().__init__(master)
        self.title("Find out what accesses this address - Passive X-Ref Scanner")
        ...
        ttk.Label(self, text=f"Target Address: 0x{addr:X}  ({type_name})", font=HEADFONT, padding=(10, 10, 10, 0)).pack(anchor="w")
        ttk.Label(
            self, padding=(10, 4, 10, 4), wraplength=780, justify="left", foreground=MUTED,
            text="Bu araç, hedef sürece sızıp debugger bağlamak (ve çökmelere yol açmak) yerine, "
                 "ana modülün kod (.text) bölümlerini pasif olarak tarar ve bu adrese doğrudan "
                 "erişen veya atıfta bulunan (X-Ref) tüm Assembly komutlarını listeler."
        ).pack(fill="x")

        # Progress indicator
        self.progress_frame = ttk.Frame(self, padding=(10, 2, 10, 6))
        self.progress_frame.pack(fill="x")
        self.status_lbl = ttk.Label(self.progress_frame, text="Tarama bekleniyor. 'Pasif Taramayı Başlat' butonuna basın.", foreground=MUTED)
        self.status_lbl.pack(side="left")
        self.progress = ttk.Progressbar(self.progress_frame, orient="horizontal", length=200, mode="determinate")
        self.progress.pack(side="right", padx=10)

        # Main Layout: Top matches list, bottom disassembly details
        ...
        self.tree = ttk.Treeview(list_frame, columns=("addr", "insn", "bytes"), show="headings", selectmode="browse")
        self.tree.heading("addr", text="Instruction Address", anchor="w")
        self.tree.heading("insn", text="Assembly Instruction", anchor="w")
        self.tree.heading("bytes", text="Opcodes (Bytes)", anchor="w")
        self.tree.column("addr", width=150, anchor="w")
        self.tree.column("insn", width=350, anchor="w")
        self.tree.column("bytes", width=250, anchor="w")
        stripe(self.tree)
        ...
        # Output Text Frame (Disassembly & AOB)
        ...
        btns.pack(fill="x")
        self.start_btn = ttk.Button(btns, text="▶ Pasif Taramayı Başlat", style="Accent.TButton", command=self._start)
        ...
        if Cs is None:
            self._log("[!] Capstone kütüphanesi yüklü değil - kod çözülemeyecek.")
    ...
    def _start(self):
        ...
        self.tree.delete(*self.tree.get_children())
        self.status_lbl.config(text="Kod bölümleri pasif olarak taranıyor...", foreground=YELLOW)
        self.progress["value"] = 0
        threading.Thread(target=self._run_passive_scan, daemon=True).start()
    ...
    def _run_passive_scan(self):
        ...
        def update_progress(percent):
            ...
            self.after(0, lambda p=percent: self._update_ui_progress(p))

        try:
            matches = self._scan_xrefs_logic(bool(is64), update_progress)
            self.after(0, lambda m=matches, is64=bool(is64): self._finish_scan(m, is64))
        except Exception as e:
            self.after(0, lambda err=str(e): self._finish_error(f"Tarama hatası: {err}"))

    def _update_ui_progress(self, percent):
        self.progress["value"] = percent

    def _scan_xrefs_logic(self, is64, update_progress_cb):
        if Cs is None:
            return []
        ...
        regions = []
        for base, size, protect, kind in winmem.iter_regions(self.process_handle, kinds=("image", "private"), writable_only=False):
            # Executable check (PAGE_EXECUTE, PAGE_EXECUTE_READ, PAGE_EXECUTE_READWRITE, PAGE_EXECUTE_WRITECOPY)
            is_exec = bool(protect & (0x10 | 0x20 | 0x40 | 0x80))
            if is_exec:
                if main_exe_base > 0:
                    # Sadece hızı artırmak adına ana modül içindeki kod bölgelerini hedefleriz
                    if base >= main_exe_base and base < main_exe_base + main_exe_size:
                        regions.append((base, size))
                else:
                    regions.append((base, size))
                    
        total_size = sum(sz for _, sz in regions)
        if total_size == 0:
            return []
        ...
        for base, size in regions:
            ...
            while pos < end:
                ...
                try:
                    for insn in md.disasm(data, pos):
                        try:
                            operands = insn.operands
                        except Exception:
                            try:
                        ...
                        for op in operands:
                            if op.type == X86_OP_MEM:
                                ...
                                # x64 RIP-relative references (e.g. [rip + 0x123])
                                if is64 and getattr(mem, "base", 0) == X86_REG_RIP:
                                    resolved_addr = insn.address + insn.size + mem.disp
                                # x86 and flat absolute references (e.g. [0x00401000])
                                elif getattr(mem, "base", 0) == 0 and getattr(mem, "index", 0) == 0:
                                    resolved_addr = mem.disp
                                    
                                if resolved_addr is not None and resolved_addr == self.target_addr:
                                    matches.append({
                ...
                pos += len(data)
                scanned += len(data)
                percent = int((scanned / total_size) * 100)
                update_progress_cb(percent)
                
        return matches

    def _finish_scan(self, matches, is64):
        self.progress["value"] = 100
        self.start_btn.config(state="normal")
        
        if not matches:
            self.status_lbl.config(text="Eşleşen pasif kod referansı bulunamadı.", foreground=YELLOW)
            self._log("[*] Kod bölümlerinde bu adrese doğrudan erişen statik/RIP-relative bir komut bulunamadı.")
            return

        self.status_lbl.config(text=f"Tarama tamamlandı! {len(matches)} referans bulundu.", foreground=OK)
        for i, m in enumerate(matches):
            addr_str = f"0x{m['address']:X}"
            insn_str = f"{m['mnemonic']} {m['op_str']}"
            bytes_str = " ".join(f"{b:02X}" for b in m["bytes"])
            self.tree.insert("", "end", values=(addr_str, insn_str, bytes_str))
    ...
    def _on_select_match(self, event):
        ...
        self.status_lbl.config(text="Seçilen komut için AOB imzası hesaplanıyor...", foreground=YELLOW)
        ...
        if err:
            ...
            self.status_lbl.config(text="İmzalar hesaplandı! Panoya kopyalayabilirsiniz.", foreground=OK)
            
            # Varsayılan olarak panoya güvenli olan uzun versiyonu kopyalayalım
            self.clipboard_clear()
