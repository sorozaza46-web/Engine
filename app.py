...
class SheetOnion(tk.Tk):
    ...
    def __init__(self):
        ...
        # Arka planda çalışacak Freeze (Dondurma) döngüsü
    ...
    def _build_table(self):
        ...
        wrap = ttk.Frame(frame)
        # Tabloya Freeze (Kilit) kolonu eklendi
        ...
        self.table.bind("<space>", self._toggle_freeze_space) # Boşluk tuşu ile de kilitlenebilir
    ...
    def _build_context_menu(self):
        ...
        self.popup_menu.add_command(label="Find out what writes to this address", command=lambda: self._find_offsets(mode="writes"))
        self.popup_menu.add_command(label="Find out what accesses this address", command=lambda: self._find_offsets(mode="accesses"))
    ...
    def _find_offsets(self, mode="writes"):
        sel = self.table.selection()
        if not sel: return
        item = sel[0]
        info = self._rows.get(item)
        if info:
            OffsetViewerDialog(self, info["addr"], info["type"], mode)
    ...
    def _toggle_freeze(self, item):
        if item in self._rows:
            ...
            # Eğer dondurulduysa, o anki mevcut değeri kilit değeri olarak kaydet
            if info["frozen"]:
                current_values = self.table.item(item, "values")
                val_now = current_values[4]
                if val_now not in ("?", "??"):
                    info["freeze_val"] = val_now
                else:
                    info["freeze_val"] = "100" # Fallback varsayılan kilit değeri
                self.table.set(item, "freeze", "[ X ]")
                self._set_status(f"Address 0x{info['addr']:X} frozen at value {info['freeze_val']}")
            else:
                info["freeze_val"] = None
                self.table.set(item, "freeze", "[  ]")
                self._set_status(f"Address 0x{info['addr']:X} unfrozen.")
    ...
    def _table_double(self, event):
        ...
        if col == "#1": # Freeze Sütununa çift tıklandıysa
    ...
    def _do_edit_value(self):
        ...
        if ok and info["frozen"]:
            info["freeze_val"] = new # Kilit aktifken değer değiştirilirse yeni kilit değeri yap
        if not ok: 
            messagebox.showerror("Sheet Onion", "Write failed.")

    def _freeze_loop(self):
        """ Arka planda kilitli [X] adresleri sürekli oyuna yazan yüksek hızlı döngü """
                ...
                for item, info in list(self._rows.items()):
                    if info["frozen"] and info["freeze_val"] is not None:
                        try:
                            ...
                            pass
            time.sleep(0.1) # 100ms aralıkla sürekli dondur (Oyunun kodu ezemesin)
        ...
        if self.scanner.handle and not self.scanning:
            for item, info in self._rows.items():
                ...
                if cur and cur[4] != text:
                    # Tablodaki değer güncellemesini yap (Kutucuk ve diğer bilgileri koruyarak)
...
class OffsetViewerDialog(tk.Toplevel):
    def __init__(self, master, addr, type_name, mode="writes"):
        super().__init__(master)
        title_text = "What writes to this address" if mode == "writes" else "What accesses this address"
        ...
        if Cs is None:
            ...
            self._log("SYS", "Lütfen terminalden 'conda env update' veya 'pip install capstone' çalıştırın.")
    ...
    def _log(self, op, msg):
        ...
        if op == "SYS":
            ...
            self.text.insert("end", f"[ASM] {op:<28} -> {msg}\n")
    ...
    def _interpret_instruction(self, mnemonic, op_str):
        ...
        if mnem == "MOV":
            if "[" in op.split(",")[0]:
                return f"MOV: Sağdaki değeri sol taraftaki hafıza adresine YAZIYOR. (Can/Mermi güncellendi)."
            return f"MOV: Sol taraftaki işlemci register'ına sağdaki adresten veri OKUYOR."
        elif mnem in ("SUB", "DEC"):
            return f"{mnem} (Eksiltme): Hedef adresteki değer AZALTILIYOR! Hasar alma veya mermi tüketimi olabilir."
        elif mnem in ("ADD", "INC"):
            return f"{mnem} (Artırma): Hedef adresteki değer ARTIRILIYOR! Can basma veya para kazanma kodu olabilir."
        elif mnem == "CMP":
            return "CMP (Karşılaştırma): Bellekteki değeri kontrol ediyor (Örn: Can bitti mi?)."
        elif mnem == "XOR" and "0" in op:
            return "XOR: Değeri sıfırlıyor (Büyük ihtimalle bitti veya öldü)."
        
        return f"{mnem} yönergesi tetiklendi, bellek üzerinde işlem yapıyor."

    def _real_debug_loop(self):
        if Cs is None:
            time.sleep(1)
            self.after(0, lambda: self._log("MOV [EAX+14C], EDX", "Simüle: Can/Mermi azaltma kodu algılandı."))
            return
            ...
            try:
                ...
                if raw_code and raw_code != last_bytes:
                    ...
                    for insn in md.disasm(raw_code, self.target_addr):
                        ...
                        if self.mode == "writes" and insn.mnemonic in ("mov", "sub", "add", "dec", "inc", "xor"):
                            self.after(0, lambda a=asm_code, c=comment: self._log(a, c))
                        elif self.mode == "accesses":
                            self.after(0, lambda a=asm_code, c=comment: self._log(a, c))
                        break
            except Exception:
                pass
