...
class SheetOnion(tk.Tk):
    ...
    def _generate_aob_by_size(self, addr, size, use_wildcards=True):
        """Belirtilen uzunlukta maskelenmiş (Wildcard) veya ham (Birebir) AOB imzası üretir."""
        ...
        aob_parts = []
        mask_threshold = int(size * 0.75)
        
        for idx, b in enumerate(buf):
            # Eğer wildcard isteniyorsa akıllı maskelemeyi devreye sok
    ...
    def _show_results_context_menu(self, event):
        """Sonuç listesinde sağ tıklandığında açılan gelişmiş 32/64 Maskeli ve Ham AOB seçim menüsü"""
        ...
        # 32-Byte Alt Seçenekleri
        res_menu.add_command(label="🎯 32-Byte AOB Bul - Maskelenmiş (??)", command=lambda: self._generate_aob_by_size(addr, 32, use_wildcards=True))
        res_menu.add_command(label="📄 32-Byte AOB Bul - Ham (Birebir)", command=lambda: self._generate_aob_by_size(addr, 32, use_wildcards=False))
        res_menu.add_separator()
        
        # 64-Byte Alt Seçenekleri
        res_menu.add_command(label="🚀 64-Byte AOB Bul - Maskelenmiş (??)", command=lambda: self._generate_aob_by_size(addr, 64, use_wildcards=True))
        res_menu.add_command(label="📂 64-Byte AOB Bul - Ham (Birebir)", command=lambda: self._generate_aob_by_size(addr, 64, use_wildcards=False))
