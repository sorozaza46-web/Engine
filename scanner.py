...
class Scanner:
    ...
    def first_scan(self, text_value):
        ...
        # Values that straddle a chunk boundary would otherwise be missed,
        # so each read overlaps the previous one by (max needle length - 1)
        # bytes. A match is only a "duplicate" (already reported last
        # iteration) if it lies entirely inside that overlap window; matches
        # that straddle the boundary end past the overlap and are new.
        max_needle_len = max((len(p) if not is_regex else 64 for _, p, _, is_regex in needles), default=1)
        ...
        for base, size, _, _ in winmem.iter_regions(self.handle, self.scan_kinds, self.writable_only):
            ...
            while pos < end:
                ...
                active_overlap = 0 if first_chunk else overlap

                for name, pattern, align, is_regex in needles:
                    if is_regex:
                        for match in pattern.finditer(data):
                            idx = match.start()
                            mlen = len(match.group())
                            if idx + mlen <= active_overlap:
                                continue
                            addr = pos + idx
                            if addr % align == 0:
                                if append_match(addr, name, mlen, data, idx): break
                    else:
                        idx = data.find(pattern)
                        while idx != -1:
                            if idx + len(pattern) <= active_overlap:
