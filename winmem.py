...
# =====================================================================
# PEB BYPASS / HIDE DEBUGGER STRUCTS & FUNCTIONS
# =====================================================================
...
def hide_debugger_peb(handle, is64):
    try:
        ...
        # 1. BeingDebugged bayrağını temizle (offset +2, 1 byte)
        write_bytes(handle, peb_addr + 2, bytes([0]))
        
        # 2. NtGlobalFlag değerini temizle (offset 0xBC (x64) veya 0x68 (x86), 4 bytes)
        flag_offset = 0xBC if is64 else 0x68
        write_bytes(handle, peb_addr + flag_offset, bytes([0, 0, 0, 0]))
...
def catch_memory_access(pid, address, size_bytes, is64, on_hit, stop_event, on_status=None):
    def status(msg):
        if on_status:
            try: on_status(msg)
    ...
    if hProcess:
        hide_debugger_peb(hProcess, is64)
    ...
    # EFlags ve donanımsal kayıtçıları aynı anda çekmek için birleşik Context Flag'leri
    CONTEXT_FULL_64 = CONTEXT_DEBUG_REGISTERS_64 | 0x00100001
    CONTEXT_FULL_32 = CONTEXT_DEBUG_REGISTERS_32 | 0x00010001

    try:
        while not stop_event.is_set():
            ev = DEBUG_EVENT()
            if not kernel32.WaitForDebugEvent(ctypes.byref(ev), 50):
                continue
                ...
                if exc_code == EXCEPTION_SINGLE_STEP:
                    ...
                    if handle:
                        try:
                            if is64:
                                buf, ctx = alloc_context64()
                                ctx.ContextFlags = CONTEXT_FULL_64
                                if kernel32.GetThreadContext(handle, ctypes.addressof(ctx)):
                                    hit_addr = ctx.Rip
                                    # DÜZELTME: Resume Flag (RF) set et. CPU'nun aynı komutta takılmasını önler!
                                    ctx.EFlags |= 0x10000
                                    kernel32.SetThreadContext(handle, ctypes.addressof(ctx))
                            else:
                                ctx = CONTEXT32()
                                ctx.ContextFlags = CONTEXT_FULL_32
                                if kernel32.GetThreadContext(handle, ctypes.byref(ctx)):
                                    hit_addr = ctx.Eip
                                    # Resume Flag (RF) set et
                                    ctx.EFlags |= 0x10000
                                    kernel32.SetThreadContext(handle, ctypes.byref(ctx))
                        finally:
                            kernel32.CloseHandle(handle)
                    
                    if hit_addr is not None and on_hit:
                        on_hit(hit_addr)
