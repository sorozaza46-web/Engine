...
# ---------------------------------------------------------------------
# Purpose: find the actual CPU instruction (EIP/RIP) that reads or
# writes a given memory address, by attaching as a debugger and setting
# a hardware breakpoint (Dr0/Dr7) on that address. This is what makes a
# real *code* AOB signature possible (masking only the address-dependent
# bytes of that instruction) instead of signing the raw data value,
# which changes every time the value changes.
#
# NOTE: this touches low-level Windows debugger internals (DEBUG_EVENT,
# CONTEXT). The struct layouts below match the documented Microsoft
# headers (winnt.h) exactly, but this module can only be import-checked
# here, not exercised against a real Windows process. Test against
# target.py first.
# =====================================================================
...
class _DEBUG_EVENT_UNION(ctypes.Union):
    _fields_ = [
        ...
        # Other union members (CreateProcess/ExitThread/ExitProcess/
        # LoadDll/UnloadDll/DebugString/RipInfo) are larger but we never
        # need their fields - this padding just has to be >= the biggest
        # one so WaitForDebugEvent never writes past the union.
...
# ---- CONTEXT (x86, 32-bit) - full, standard 716-byte layout ----
...
# ---- CONTEXT (x64) - only the fields we touch (up to Rip) are typed;
# the rest (XSAVE area / vector registers / last-branch info) is opaque
# padding so the struct's total size matches the real 1232-byte CONTEXT
# that GetThreadContext/SetThreadContext expect. x64 CONTEXT must be
# 16-byte aligned in memory - see alloc_context64() below.
CONTEXT64_SIZE = 1232
...
def alloc_context64():
    """x64 CONTEXT must live at a 16-byte aligned address or
    Get/SetThreadContext can fail. Over-allocate and hand back a
    CONTEXT64 view into the aligned portion; caller must keep the
    returned buffer alive as long as the view is used."""
...
def _dr7_bits(size_bytes, is64):
    # RW field: 01 = write only, 11 = read or write. LEN field encodes
    # the watched size: 00=1 byte, 01=2 bytes, 11=4 bytes, 10=8 bytes
    # (8-byte length breakpoints only exist in long/x64 mode).
    rw = 0b01
    if size_bytes == 1:
        length = 0b00
    elif size_bytes == 2:
        ...
        length = 0b11  # default/fallback: treat as 4-byte
    local_enable = 0b1  # L0
...
def catch_memory_access(pid, address, size_bytes, is64, timeout_ms=20000, on_status=None):
    """Attach as a debugger, set a hardware breakpoint on `address`, and
    wait for a thread to actually touch it. Returns the EIP/RIP of the
    instruction that did (an int), or None on timeout/failure.

    `on_status` is an optional callback(str) for progress messages.
    Safe to call while a separate OpenProcess() read/write handle to
    the same process is open elsewhere - DebugActiveProcess doesn't
    need or return a handle of its own.
    """
    def status(msg):
        if on_status:
            try: on_status(msg)
    ...
    try:
        while _time.time() < deadline:
            ...
            if not kernel32.WaitForDebugEvent(ctypes.byref(ev), min(remaining_ms, 500)):
                continue  # timeout on this wait slice, loop and recheck deadline

            code = ev.dwDebugEventCode
            cont_status = DBG_CONTINUE

            if code == CREATE_THREAD_DEBUG_EVENT:
                # A brand-new thread: it needs the breakpoint too.
